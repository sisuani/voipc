    ...
    , ui(new Ui::MainWindow)
{
   ...
   //          this, SLOT(new_incoming_im(QString,QString)), Qt::QueuedConnection);
    ...
    connect(ui->registerButton, SIGNAL(clicked()), SLOT(registerClicked()));
...
void MainWindow::voipcInitialize()
{
    ui->registeredLabel->setText("....");
    QByteArray tempStun;
    pj_status_t status;
    ...
    const QString domain = ui->domainLineEdit->text();
...
void MainWindow::callClicked()
{
    ...
    if(ui->callButton->text() == "Llamar") {
        ...
        if(status != PJ_SUCCESS) {
            ...
            activeCalls << call_id;
            ui->callButton->setText("Cortar");
        }
        activeCallsMutex.unlock();
    } else if(ui->callButton->text() == "Atender") {
        activeCallsMutex.lock();
        if (activeCalls.empty()) {
            ui->callButton->setText("Llamar");
            activeCallsMutex.unlock();
            return;
        }
    ...
    } else if(ui->callButton->text() == "Cortar") {
        activeCallsMutex.lock();
        if(activeCalls.empty()) {
            ui->callButton->setText("Llamar");
            activeCallsMutex.unlock();
...
void MainWindow::setCallButtonText(QString text)
{
